const fs = require('fs');
let dash = fs.readFileSync('src/components/StudentDashboard.tsx', 'utf-8');

// Replace recharts imports
dash = dash.replace("import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, LineChart, Line, CartesianGrid } from 'recharts';", "");
...
export function StudentDashboard() {
  ...
  const [streakDays] = useState(() => {
    ...
    const streak = localStorage.getItem('study_streak') || '1';
    const today = new Date().toDateString();
    
    if (lastActive !== today) {
      localStorage.setItem('last_active_date', today);
      // Real streak logic would be more complex, keeping it simple for the UI
    }
...
fs.writeFileSync('src/components/StudentDashboard.tsx', newDashboard);
